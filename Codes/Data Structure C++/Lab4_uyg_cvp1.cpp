...
list *head;        // bu listedeki tüm elemanlara eriþeceðimiz 
					//head pointer  tanýmlamasýdýr ve global olarak tanýmlamamýz gerekir.
...
void list::ekle(int sayi)
{
 ...
 newlist->next=0;
 list *p=head;             // listede dolaþacak bir pointer tanýmlanýr.
 if(p==0) 
  head=newlist;
 else
 {
...
void list::sil(int x){
 ...
 {   p=p->next;                      // diðer durumlarýn silindiði yer .
  while(p)
  {                       
   if(p->data==x) 
   {
